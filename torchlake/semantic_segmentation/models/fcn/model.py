...
class FCNLegacy(nn.Module):
    def __init__(
        ...
        Args:
            ...
            num_skip_connection (int, optional): number of skip connection, e.g. 3 means FCN-8s, 2 means FCN-16s, 1 means FCN-32s. Defaults to 3.
            ...
            [
                ...
                *[
                    ...
                    for in_c, out_c in (
                        ...
                        (256, 128),  # pool3 -> pool2
                        (128, 64),  # pool2 -> pool1
                    )[: num_skip_connection - 1]
                ],
                # final upsample
                UpSamplingLegacy(
                    [512, 512, 256][num_skip_connection - 1],  # 32s, 16s, 8s
                    output_size,
                    # e.g. 3 => 8s => 8, 2 => 16s => 16
                    kernel=2 ** (6 - num_skip_connection),
                    # e.g. 3 => 8s => 8, 2 => 16s => 16
                    stride=2 ** (6 - num_skip_connection),
                ),
            ]
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        layers_no = [f"{i}_1" for i in range(5, 5 - self.num_skip_connection, -1)]
...
class FCN(nn.Module):
        ...
        self.pool_convs = nn.ModuleList(
            [
                ...
                *[
                    ...
                    for dim in [512, 512, 256, 128, 64][1:num_skip_connection]
                ],
            ]
        )
        self.upsamples = nn.ModuleList(
            [
                # intermediate upsample
                *[UpSampling(output_size, output_size, 4, stride=2)]
                * (num_skip_connection - 1),
                # final upsample
                UpSampling(
                    output_size,
                    output_size,
                    # e.g. 3 => 8s => 16, 2 => 16s => 32
                    kernel=2 ** (7 - num_skip_connection),
                    # e.g. 3 => 8s => 8, 2 => 16s => 16
                    stride=2 ** (6 - num_skip_connection),
                ),
            ]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # feature fusion
        y = self.pool_convs[0](self.conv(features.pop()))  # 32x
        # y = self.pool_convs[0](features.pop())  # 32x
        for upsample, pool_conv in zip(self.upsamples[:-1], self.pool_convs[1:]):
            y = upsample(y)
