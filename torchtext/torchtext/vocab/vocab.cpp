...
Vocab::Vocab(StringList tokens, const c10::optional<int64_t>& default_index)
  ...
  for (auto& token : tokens) {
    ...
    TORCH_CHECK(stoi_[id] == -1, "Duplicate token found in tokens list: " + token);
    _add(std::move(token));
  }
}
...
int64_t Vocab::__getitem__(const c10::string_view& token) const {
  ...
  TORCH_CHECK(
      ...
      "Token " + std::string(token) + " not found and default index is not set");
...
void Vocab::append_token(std::string token) {
  ...
  TORCH_CHECK(
      stoi_[id] == -1,
      "Token " + token + " already exists in the Vocab with index: " + std::to_string(stoi_[id]));
  _add(std::move(token));
}
  ...
  for (size_t i = 0; i < indices.size(); i++) {
    TORCH_CHECK(
        ...
        "Specified index " + std::to_string(indices[i]) + " at position " +
            std::to_string(i) + " is out of bounds for vocab of size " + std::to_string(__len__()));
  }
...
VocabStates _serialize_vocab(const c10::intrusive_ptr<Vocab>& self) {
  ...
  return std::make_tuple(self->version_str_, std::move(integers), std::move(strings), std::move(tensors));
}
...
c10::intrusive_ptr<Vocab> _deserialize_vocab(VocabStates states) {
  ...
  TORCH_CHECK(version_str.compare("0.0.2") >= 0, "Found unexpected version for serialized Vocab: " + version_str);
